<?
	...
	if($type=='view' && $uid){
		...
		if($saleChk){
			...
			elseif($reduction == '국민기초생활보장수급(주거/교육)')		$rate = '50';
			elseif($reduction == '장애인(1~3급)')								$rate = '50';
			elseif($reduction == '장애인(4~6급)')								$rate = '20';
			elseif($reduction == '국가보훈대상(본인 및 직계가족)')		$rate = '50';
			elseif($reduction == '차상위계층')									$rate = '50';
			elseif($reduction == '다자녀가족')									$rate = '50';
			elseif($reduction == '경로')											$rate = '50';
			elseif($reduction == '직원')											$rate = '40';
			else																			$rate = '';
...
function reAmtSet(m){
	...
	if(m == 'reAmt'){
		reEtc = parseInt(payAmt) - parseInt(reAmt);
		$("#reEtc").val(reEtc);

	}else if(m == 'reEtc'){
		reAmt = parseInt(payAmt) - parseInt(reEtc);
		$("#reAmt").val(reAmt);
	}
}
...
?>
	...
	<div class='mCadeTit02' style='margin:30px 0 3px 0;'>환불/취소 정보</span></div>
	<table cellpadding='0' cellspacing='0' border='0' width='100%' class='zTable'>
		<tr>
			<th>상태</th>
			<td colspan='3'>
				<table cellpadding='0' cellspacing='0' border='0'>
					<tr height='40'>
						...
						<td style='padding:0 0 0 40px;'>
							<div class="squaredThree">
								<input type="checkbox" value="환불신청" id="sT3" name="reFund" onclick='setChkBox(this.name,2);' <?if($reFund == '환불신청'){echo 'checked';}?>>
								<label for="sT3"></label>
							</div>
		...
		<tr>
			<th>이용금액</th>
			<td colspan='3'><span id='reUseTxt'><?=$reUseTxt?></span><input type='hidden' name='reUse' id='reUse' value='<?=$reUse?>'></td>
		</tr>
	...
	?>
		...
		<tr>
			<th width='17%'>환불금액</th>
			<td width='33%'><input name="reAmt" id="reAmt" style="width:120px;" type="text" value="<?=$reAmt?>" class='numberOnly' onblur="reAmtSet(this.name);">원</td>
			<th width='17%'>환불수수료</th>
			<td width='33%'><input name="reEtc" id="reEtc" style="width:120px;" type="text" value="<?=$reEtc?>" class='numberOnly' onblur="reAmtSet(this.name);">원</td>
		</tr>

		<tr>
			<th>처리일</th>
			<td colspan='3'><input type='text' name='reDate' id='fpicker1' value='<?=$reDate?>' readonly onchange='refundChk();'></td>
		</tr>

		<tr>
			<th>비고</th>
			<td colspan='3'><textarea name='reMemo' style='width:100%;height:100px;border:1px solid #ccc;resize:none;'><?=$reMemo?></textarea></td>
		</tr>
	</table>
<?
	if($payMode == '단말기'){
	...
	<table cellpadding='0' cellspacing='0' border='0' width='100%'>
		<tr>
			<td align='center' style='padding:30px 0;'>
				<a href="javascript:refund();" class='big cbtn green'>환불/취소</a>&nbsp;&nbsp;
