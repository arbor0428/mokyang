...
function programChk(){
	...
	$.post('json.php',{'cade01':cade01}, function(c1){
		//강습반명 selectbox 초기화
		$('#f_title').empty();
		$('#f_title').append("<option value=''>:: 직접입력 ::</option>");
...
function strChk(){
	idx = $("#f_title option").index($("#f_title option:selected"));
	if(idx == 0){
		$('#f_str').show();
	}else{
		$('#f_str').hide();
			...
			<table cellpadding='0' cellspacing='0' border='0' width='100%' class='gTable'>
				<tr>
					...
					<td colspan='3'>
						...
						<select name='f_title' id='f_title' onchange='strChk();' style='border:1px solid #ccc;height:30px;'>
						...
						<input name="f_str" id="f_str" type="text" style="width:200px;<?if($f_title){echo 'display:none;';}?>" value="<?=$f_str?>" onkeypress="if(event.keyCode==13){go_search();}" placeholder="강습반명을 입력해 주세요.">
					</td>
				</tr>
