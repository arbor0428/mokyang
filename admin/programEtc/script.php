...
//	getDate = $('#fpicker1').val();
	...
	//프로그램정보
	if(title){
		$.post('json_period03.php',{'rate':rate,'year':year,'season':season,'c1':c1,'period':period,'title':title,'fitnessDate':fitnessDate}, function(req){
			req = urldecode(req);
			parData = JSON.parse(req);		
...
function check_form(){
	...
	if(isFrmEmptyModal(form.etcAmt,"수강료를 입력해 주십시오."))	return;
