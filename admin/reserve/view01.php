...
<table cellpadding='0' cellspacing='0' border='0' width='100%' class='zTable'>
	<tr>
		<th width='15%' style='border-bottom:2px solid #8d8d8d;'><?=$ico01?> 사용기간</th>
		<td width='85%' style='border-bottom:2px solid #8d8d8d;'><?=$sDate01Txt?></td>
	</tr>
	...
	<tr>
		...
		<?
			for($c=0; $c<count($HallArr01); $c++){
				$sVar = $HallArr01[$c];

				$chk = '';

				if(is_array($hallDataArr01)){
					if(in_array($sVar,$hallDataArr01))		$chk = 'checked';
				}
		?>
		<div style='width:25%;float:left;height:40px;'>
			<div class="squaredThree">
				<input type="checkbox" value="<?=$sVar?>" id="hallChk01<?=$c?>" name="hallChk01[]" <?=$chk?> disabled>
				<label for="hallChk01<?=$c?>"></label>
			</div>
