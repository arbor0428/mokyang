...
function art_show(){
	...
	sDate = $("#data05 option:selected").attr("data-sd");
	eDate = $("#data05 option:selected").attr("data-se");
	$("#fpicker1").val(sDate);
	$("#fpicker2").val(eDate);
}
</script>
				...
				<tr> 
					...
					?>
						<select name='data05' id='data05' onchange='art_show();'>
							<option value='' data-sd='<?=$startDate?>' data-se='<?=$endDate?>'>:: 공연정보선택 ::</option>
					<?
						for($i=0; $i<$anum; $i++){
							$arow = mysql_fetch_array($aresult);
							$auid = $arow['uid'];
							$atitle = $arow['title'];
							$asDate = date('Y-m-d',$arow['startTime']);
							$aeDate = date('Y-m-d',$arow['endTime']);

							if($data05 == $auid)	$chk = 'selected';
							else						$chk = '';

							echo ("<option value='$auid' data-sd='$asDate' data-se='$aeDate' $chk>$atitle</option>");
						}
					?>
						</select>
					</td>
					...
					<td colspan='3'>
						<table cellpadding='0' cellspacing='0' border='0'>
							<tr>
								...
								<td style='padding-left:20px;'>* 선택한 기간에 건별로 전부 등록됩니다.</td>
							</tr>
						</table>						
					</td>
				</tr>
