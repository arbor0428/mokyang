...
.view-boxs table, .view-boxs iframe, .view-boxs img {max-width:100% !important; width:100% !important;}
</style>
<?
	}
?>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
	<tr>
		<td>
			...
			?>
				<tr>
					<td colspan='3' style='font-size:1.2em;font-weight:800;'><?=$title?></td>
				</tr>

				<tr>
					<td colspan='3'><?=$reg_date?> <span style='padding:0 10px;'>|</span> <?=$name?> <span style='padding:0 10px;'>|</span> <?=number_format($hit)?></td>
				</tr>
			<?
				}
			?>
				...
				<tr height="300" valign="top">
					<td class='bbs01' colspan='3' style="padding:30px 0;">
					<?
						//뉴스(언론보도형)
						if($table_id == 'table_1639985596' && $data01){
					?>
						<div style='width:100%;margin-bottom:30px;font-size:1.2em;'><a href="<?=$data01?>" target="_blank"><?=$data01?></div>
					<?
						}
					?>
					...
					<?
						if($sDataUrl){
							$tmpArr = explode('/',$sDataUrl);
							$videoCode = $tmpArr[count($tmpArr)-1];
					?>
						<div style='width:100%;max-width:760px;margin:0 auto;'>
							<div class="iframebox">
								<iframe class="voskin_vd" src="https://www.youtube.com/embed/<?=$videoCode?>" frameborder="0" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen=""></iframe>
							</div>
						</div>
					<?
						}
